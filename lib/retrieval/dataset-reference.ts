...
const CAPABILITY_INSTRUCTION = [
  ...
  "There is a third source, broadcast_cv, which is proprietary tracking data from our own computer vision. For each processed clip it holds anonymous player and ball tracks with movement metrics, total distance, top speed, and average speed, in cv_track_metrics, joined to cv_clips. These are per anonymous track, not per identified player, and they describe individual video clips identified by cv_clips.clip_name. The units depend on the clip: meters only when the clip was calibrated against known pitch positions, otherwise image pixels, given by the distance_units and speed_units columns.",
  "Critical: cv_track_metrics and cv_clips have no link to any competition, team, or identified player. A movement metric like distance or speed for a named competition, for example the World Cup or the Premier League, can only be answered if a clip from that competition exists in cv_clips, and none does. So a question about a competition's distance or speed must filter cv_clips to that competition by clip_name and will correctly return no rows. Never sum cv_track_metrics across all clips and present it as a competition's total, and never present a movement metric for a competition that has no processed clip.",
].join(" ");
